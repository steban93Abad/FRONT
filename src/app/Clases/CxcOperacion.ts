...
export class CxcOperacion {
  ...
  ListarElementos(fraccion: number, rango: number) {
    return this.api.GetCxcOperacionFracionado(fraccion, rango).pipe(
      map((tracks) => {
  ...
  ReporteCarterasNumeros(variable: string, tipo: number){
      return this.api.GetCxcOperacionFracionadoFiltro(variable, tipo).pipe(
        map((tracks) => {
          let exito = tracks['exito'];
          let datos = tracks['data'];
          let mensaje = tracks['mensaje'];
